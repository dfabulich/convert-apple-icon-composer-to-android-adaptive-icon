 ...
 * @returns {Promise<{r: number, g: number, b: number, a: number}>} RGBA values (0-255)
 */
export async function samplePixelColor(imagePath, x, y) {
  const image = sharp(imagePath);
  const metadata = await image.metadata();
  
  // Clamp coordinates to image bounds
  x = Math.max(0, Math.min(Math.floor(x), metadata.width - 1));
  y = Math.max(0, Math.min(Math.floor(y), metadata.height - 1));
